...
export default async function DashboardPage() {
  const [user, dashboard] = await Promise.all([getServerUser(), getDashboardServer()]);
  const { stats, recentMemories, dashboardActivity, dailyBrief } = dashboard;
  const firstName =
  ...
  return (
    <div>
      <Topbar title={`Good morning, ${firstName}`} description="Here's what your memory surfaced today." />
          ...
          <div className="mt-3 divide-y divide-line">
            {recentMemories.length === 0 && (
              <p className="px-5 py-8 text-center text-sm text-muted">
                Nothing here yet — add your first memory from the Inbox.
              </p>
            )}
